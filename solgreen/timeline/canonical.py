...
class CanonicalSample(BaseModel):
    ...
    timestamp_axis: datetime = Field(
        description="Instante del eje temporal canónico (UTC)."
    )
    ...
    telemetry_grid_power_w: float | None = Field(
        default=None,
        description="Potencia activa total de red (W). Solo presente si source=telemetry o source=merged."
    )
